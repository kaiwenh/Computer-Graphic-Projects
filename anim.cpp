...
void drawLeg(float angle, int k)
{
    float theta, leg_segment = 1.5;
    for (int i = 0; i < 2; i++)
    {
        theta = 10*sin(angle*DegreesToRadians*TIME)+10*k;
        model_view *= Translate(0, 0.7, 0.08);
        model_view *= RotateX(theta);
        model_view *= Translate(0, -0.7, -0.08);
        mvstack.push(model_view);
        model_view *= Scale(0.2, leg_segment, 0.2);
        drawCube();
...
void display(void)
{
    ...
    //draw tree trunk with 8 parts
    double l_segment = 3, sway_speed = 100;
    for (int i = 0; i < 8; i++)
    {
        set_colour(0.8f, 0.0f, 0.0f);
        model_view *= RotateZ(2*sin(sway_speed*DegreesToRadians*TIME));
    ...
    //draw leave
    set_colour(0.0f, 3.5f, 0.0f);
    model_view *= Translate(0, 1, 0);
    model_view *= Scale(2, 2, 2);
    drawSphere();
    
    //draw head
    set_colour(0.576, 0.44, 0.86);
    model_trans *= RotateY(10.0*TIME);
    model_trans *= Translate(0.0f, 2.5*sin(30*DegreesToRadians*TIME), -15.0f);
    ...
    model_trans *= Translate(3, 0, 0);
    mvstack.push(model_trans);
    mvstack.push(model_trans);
    model_trans *= Scale(4, 1, 1);
    model_view = view_trans * model_trans;
    drawCube();
    ...
    model_trans *= Translate(-1.8, -1, 0.5);
    model_view = view_trans * model_trans;
    ...
    model_trans = mvstack.pop();
    model_trans *= Translate(3, 0, 0);
    model_trans *= Scale(2, 1, 1);
    model_view = view_trans * model_trans;
    drawSphere();
    
    //draw wings
    set_colour(0.5, 0.5, 0.5);
    model_trans *= Translate(-2, 0.35, 2.5);
    mvstack.push(model_trans);
    model_trans *= Translate(0, -0.1, -2);
    model_trans *= RotateX(20*sin(100*DegreesToRadians*TIME));
    model_trans *= Translate(0, 0.1, 2);
    model_trans *= Scale(0.8, 0.2, 4);
    model_view = view_trans * model_trans;
    drawCube();
